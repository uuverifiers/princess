...
class CCUSolver[TERM, FUNC] {
  ...
  def solveaux(terms : List[Int], domains : Map[Int, Set[Int]], goals : List[List[(Int, Int)]], functions : List[(FUNC, List[Int], Int)], debug : Boolean) : (Option[Array[Int]], Map[(Int, Int), Int]) = {
    ...
    var assignments = scala.collection.mutable.Map() : scala.collection.mutable.Map[(Int, Int), Int]
    ...
    def printModel(Model : Array[Int], EndColumn : Int) = {
    ...
    def T(Column : Int, Row : Int) : Int = 1 + Column*ROWS*BITS + slackvars.take(Column).sum + Row*BITS
    ...
    def AddInitialColumn() = {
      ...
      for (t <- terms) {
        if ((domains.getOrElse(t, Set())).size <= 1) {
  ...
  def solve(Terms : List[TERM], Domains : Map[TERM, Set[TERM]], Goals : List[List[(TERM, TERM)]], Functions : List[(FUNC, List[TERM], TERM)]) : Option[Map[TERM, TERM]]= {
    var TermToInt = scala.collection.mutable.Map() : scala.collection.mutable.Map[TERM, Int] 
    var IntToTerm = scala.collection.mutable.Map() : scala.collection.mutable.Map[Int, TERM] 
    ...
    val problem = (Terms.map(TermToInt), domains, goals, functions) 

    val result = solveaux(terms, domains, goals, functions, true) : (Option[Array[Int]], Map[(Int, Int), Int])

    result match {
      case (Some(model), assignments) => {
        var assMap = scala.collection.mutable.Map() : scala.collection.mutable.Map[TERM, TERM]
