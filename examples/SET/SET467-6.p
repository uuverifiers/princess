%--------------------------------------------------------------------------
% File     : SET467-6 : TPTP v6.1.0. Bugfixed v2.1.0.
% Domain   : Set Theory
...
%            naming scheme).
%          : This version from [Quaife] is different from that in [Qua92].
...
%--------------------------------------------------------------------------
cnf(prove_sets_with_one_member1_1,negated_conjecture,
    ( subclass(cross_product(x,x),identity_relation) )).

cnf(prove_sets_with_one_member1_2,negated_conjecture,
    (  singleton(not_subclass_element(x,null_class)) != x )).

cnf(prove_sets_with_one_member1_3,negated_conjecture,
    (  x != null_class )).
