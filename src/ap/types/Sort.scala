...
object Sort {
  ...
  class DomPredUninterpreted(override val name : String,
                             val domPredicate : IExpression.Predicate)
        extends ProxySort(Integer) {

    override def membershipConstraint(t : ITerm) : IFormula =
      IAtom(domPredicate, List(t))

    override def membershipConstraint(t : Term)
                    (implicit order : TermOrder) : Formula =
      Atom(domPredicate, Array(LinearCombination(t, order)), order)
  }
