...
object StaticMetaStrategyLearner extends App {
  
  import SAnnealing._
  import Learner._
  import FullPortfolios._
  
  val consideredStrategies =
//   allStrategies
    List("0010004.log", "1201113.log", "0101001.log", "1210010.log", "1000001.log", "1010004.log", "1200113.log", "1110003.log", "1210112.log", "0101011.log", "1000111.log", "0001104.log", "1210014.log", "1101003.log", "1010002.log", "0001011.log", "1201103.log", "1111013.log", "1110004.log", "1110001.log", "1110103.log", "1100103.log", "1001001.log", "1011004.log", "0001113.log", "0201003.log", "1001112.log", "1110014.log", "1110112.log", "0000001.log", "1201014.log", "1211010.log", "0210004.log", "1101001.log", "0110003.log", "1201001.log", "1101101.log", "0001001.log", "1200001.log", "1201003.log", "1010111.log", "0111004.log", "1101013.log", "1001011.log", "1201102.log", "1100003.log", "1200111.log", "0110004.log")
//    List("1201103.log", "1001101.log", "1010102.log", "1010010.log", "1010001.log", "1010011.log", "1001001.log", "1000111.log", "1001102.log", "1101101.log", "1110001.log", "1010002.log", "1001004.log", "1110110.log", "1000001.log", "1010101.log", "1110111.log", "1111004.log", "1111010.log", "1010014.log", "1010110.log", "1110014.log", "1000011.log", "0010004.log", "1011004.log", "0110004.log", "0000001.log", "1101001.log", "1201003.log", "1111001.log", "0101001.log", "1011001.log", "0111004.log", "1110004.log", "1100001.log", "1000101.log", "1100103.log", "1110103.log", "1001002.log", "1000102.log", "1010111.log", "0001001.log", "1010004.log", "1001011.log", "1011101.log", "1110003.log", "0011004.log", "1200113.log")


  val metaStrategyRand = new Randomiser[MetaStrategy] {
    private val rand = new scala.util.Random(1235)
    
    private def randomTimeout =
      (rand nextInt 121) * 1000
    private def randomStrategy =
      consideredStrategies.iterator.drop(rand nextInt consideredStrategies.size).next
    
    def initialObject : MetaStrategy = List()
    def mutate(s : MetaStrategy) : MetaStrategy = {
      while (true)
        (rand nextInt 6) match {
          case 0 if (s.size < 20) => { // insert new sub-strategy
            val index = rand nextInt (s.size + 1)
            return (s take index) :::
                   List(TryStrategyCommand(randomStrategy, randomTimeout)) :::
                   (s drop index)
          }
          case 1 if (!s.isEmpty) => { // remove sub-strategy
            val index = rand nextInt s.size
            return (s take index) ::: (s drop (index + 1))
          }
          case 2 if (!s.isEmpty) => { // change timeout
            val index = rand nextInt s.size
            return s(index) match {
              case TryStrategyCommand(oldStrategy, oldTimeout) =>
                s.updated(index, TryStrategyCommand(oldStrategy, randomTimeout))
            }
          }
          case 3 if (!s.isEmpty) => { // increase/decrease timeout
            val index = rand nextInt s.size
            return s(index) match {
              case TryStrategyCommand(oldStrategy, oldTimeout) =>
                s.updated(index,
                    TryStrategyCommand(oldStrategy,
                        ((oldTimeout + ((rand nextInt 11) * 1000 - 5000)) max 0) min 120000))
            }
          }
          case 4 if (!s.isEmpty) => { // change strategy
            val index = rand nextInt s.size
            return s(index) match {
              case TryStrategyCommand(oldStrategy, oldTimeout) =>
                s.updated(index, TryStrategyCommand(randomStrategy, oldTimeout))
            }
          }
          case 5 if (s.size >= 2) => { // swap strategies
            val index = rand nextInt (s.size-1)
            val s1 :: s2 :: tail = s drop index
            return (s take index) ::: List(s2, s1) ::: tail
          }
  ...
  def printMetaStrategyInfo(s : MetaStrategy) = {
    println(s)
    println("Solves " + fofPortfolio.solvedProblemNum(s) +
            " FOF problems, average time " + fofPortfolio.avgTime(s))
    println("Solves " + cnfPortfolio.solvedProblemNum(s) +
            " CNF problems, average time " + cnfPortfolio.avgTime(s))
    println("Solves " + tffPortfolio.solvedProblemNum(s) +
            " TFF problems, average time " + tffPortfolio.avgTime(s))
    println("Solves " + tfaPortfolio.solvedProblemNum(s) +
            " TFA problems, average time " + tfaPortfolio.avgTime(s))
    println("Solves " + tfaNonlinPortfolio.solvedProblemNum(s) +
            " nonlinear TFA problems, average time " + tfaNonlinPortfolio.avgTime(s))
  }
  
  val annealer = new Annealer[MetaStrategy] {
    ...
    def cost(s : MetaStrategy) =
      ...
      tffPortfolio.cost(s) +
      tfaPortfolio.cost(s) +
      tfaNonlinPortfolio.cost(s) +
      s.size /* -
      (for ((TryStrategyCommand(_, to), i) <- s.iterator.zipWithIndex) yield to/(i+1)).sum */
   def printObj(obj : MetaStrategy) = printMetaStrategyInfo(obj)
  }
  
  val s = annealer.optimise(2000000, 0.9999, 100000, 0.4)
//  val s = annealer.optimise(500000, 0.9999, 100000, 0.4)
...
object SimulatingMetaStrategyLearner extends App {
  
  import SAnnealing._
  import SelectPortfolios.{selStrategies, selPortfolios}
  ...
  val annealer = new Annealer[SolverSimulator.MetaStrategy] {
    ...
    def cost(s : MetaStrategy) = {
      ...
      val fixedCosts =
        (if (s.commands(0).asInstanceOf[RunStrategy].strategy == "10010010.log") 0 else 1000000) +
        (if (s.commands(1).asInstanceOf[RunStrategy].strategy == "10001111.log") 0 else 500000) +
        (if (s.commands(2).asInstanceOf[RunStrategy].strategy == "12010030.log") 0 else 100000)

     solvedTimeCost + fixedCosts
...
object CoarseStrategySelector extends App {
...
/*         val solvedNum =
           (for (s <- strategies.iterator) yield portfolio.strategyResults(s)(p) match {
              case AnyTheoremResult(millis) =>
                10000.0 / (2000.0 + millis.toDouble)
              case InconclusiveResult(millis) =>
                1000.0 / (2000.0 + millis.toDouble)
              case _ => 0
            }).sum

         -(scala.math.log1p(solvedNum.toDouble) * 1000.0).toLong */
       }).sum
    
    def cost(s : List[String]) =
      cost(s, fofPortfolio) + cost(s, tffPortfolio)
      // + cost(s, cnfPortfolio) +
      //       cost(s, tfaPortfolio) + cost(s, tfaNonlinPortfolio)

    def printObj(obj : List[String]) = {
      println(obj)
      println(  "FOF: " + fofPortfolio.solvedProblemsSimple(obj).size)
//      println(  "CNF: " + cnfPortfolio.solvedProblemsSimple(obj).size)
      println(  "TFF: " + tffPortfolio.solvedProblemsSimple(obj).size)
//      println(  "TFA: " + tfaPortfolio.solvedProblemsSimple(obj).size)
//      println(  "TFA nonlin: " + tfaNonlinPortfolio.solvedProblemsSimple(obj).size)
    }
  }
...
//    printPortfolioInfo(s, cnfPortfolio)
    println
    printPortfolioInfo(s, tffPortfolio)
//    println
//    printPortfolioInfo(s, tfaPortfolio)
//    println
//    printPortfolioInfo(s, tfaNonlinPortfolio)
    ...
    for (t <- allStrategies.toSeq.sorted) {
      ...
      print(t)
      print("\t")
      for (port <- List(fofPortfolio, /* cnfPortfolio, */ tffPortfolio /*,
                        tfaPortfolio, tfaNonlinPortfolio */)) {
